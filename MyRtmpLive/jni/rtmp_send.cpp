...
{  
	...
	// linux
	struct sockaddr{
	   unsigned short sa_family; // protocol cluster AF_XXX -- AF_INET
	...
	struct sockaddr_in {
	   ...
	   struct in_addr sin_addr;           //internal address
	   unsigned char sin_zero[8];
	}
	*/
	/*连接服务器*/
	if (RTMP_Connect(m_pRtmp, NULL) == FALSE) // NetConnection  struct sockaddr_in service
...
{    
	...
	LOGI("             metaData.nFrameRate %d              ",metaData.nFrameRate);
	//发送PPS,SPS
...
got_sps_pps:
		...
		if(naluUnit.type == 0x07 || naluUnit.type == 0x08){
			LOGI("              RtmpLive pps/sps %d              ",naluUnit.type);
			goto got_sps_pps;
		}
...
JNIEXPORT void JNICALL Java_com_example_myrtmplive_MainActivity_RtmpLive(JNIEnv *env, jclass clz, jstring title)
{
    char const* local_title = env->GetStringUTFChars(title, NULL);

	LOGI("              RtmpLive start              ");
	LOGI("              RtmpLive %s              ", local_title);
	fp_send1 = fopen(local_title, "rb+"); //"cuc_ieschool.h264"

	LOGI("              RtmpLive              ");
	//初始化并连接到服务器
	//RTMP264_Connect("rtmp://localhost/publishlive/livestream");
	RTMP264_Connect("rtmp://192.168.1.9:1935/mylive/livestream"); //localhost

	LOGI("              RtmpLive connected              ");
	//发送
	RTMP264_Send();
	LOGI("              RtmpLive send finished              ");
